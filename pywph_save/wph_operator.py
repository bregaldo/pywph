...
from .utils import to_torch, get_precision_type, get_memory_available, fft, ifft, mul, modulus, PhaseHarmonics, PowerHarmonics
...
class WPHOp():
    ...
    def load_filters(self, lp_filter_cls, bp_filter_cls):
        ...
        self.psi_f = to_torch(self.psi_f, device=self.device, precision=self.precision, force_cplx_dim=True)
        self.phi_f = to_torch(self.phi_f, device=self.device, precision=self.precision, force_cplx_dim=True)
    ...
    def preconfigure(self, data, requires_grad=False, mem_chunk_factor=15, mem_chunk_factor_grad=30):
        data = to_torch(data, device=self.device, precision=self.precision, force_cplx_dim=True)
        data_size = 1
        ...
        data_wt_f = mul(data_f, self.psi_f)
        del data_f
    ...
    def apply_chunk(self, data, data_wt, chunk_id):
        if chunk_id < self.nb_chunks_wph: # Estimation of the WPH moments
            ...
            xpsi2_k2[..., 1] = -xpsi2_k2[..., 1]
            
            # Substract the mean
            xpsi1_k1 -= torch.mean(xpsi1_k1, (-2, -3), keepdim=True)
            xpsi2_k2 -= torch.mean(xpsi2_k2, (-2, -3), keepdim=True)
            
            # Compute correlation and covariance
            corr = mul(xpsi1_k1, xpsi2_k2)
            del xpsi1_k1, xpsi2_k2
            cov = torch.mean(corr, (-2, -3))
            del corr
            return cov
            ...
            # Separate real and imaginary parts of input data
            data_new = torch.zeros(data.shape[:-3] + (2,) + data.shape[-3:],
                                   dtype=data.dtype, device=data.device) # (...,2, M, N, 2)
            data_new[..., 0, :, :, 0] = data[..., 0]
            data_new[..., 1, :, :, 0] = data[..., 1]
            data_new = data_new.unsqueeze(-4).unsqueeze(-4) # (..., 2, 1, 1, M, N, 2)
            
            # Subtract the mean to avoid a bias due to a non-zero mean of the signal
            data_new -= torch.mean(data_new, (-2, -3), keepdim=True)
            
            # Convolution with scaling functions
            data_f = fft(data_new)
            del data
            data_st_f = mul(data_f, self.phi_f.unsqueeze(-4))  # (..., 2, J-3, 1, M, N, 2)
            del data_f
            data_st = ifft(data_st_f)
            del data_st_f
            
            # Compute moments
            data_st = data_st.expand(data_st.shape[:-4] + (1,) + data_st.shape[-3:]) # (..., 2, J-3, P, M, N, 2)
            data_st_p = power_harmonics(data_st, self._indices_p)
            del data_st
            
            # Substract mean
            data_st_p -= torch.mean(data_st_p, (-2, -3), keepdim=True) # (..., 2, J-3, P, M, N, 2)
            
            # Compute covariance
            data_st_p = modulus(data_st_p)
            cov = torch.mean(mul(data_st_p, data_st_p), (-2, -3))
            cov = cov.view(data_st_p.shape[:-6] + (data_st_p.shape[-6]*data_st_p.shape[-5]*data_st_p.shape[-4], 2)) # (..., 2*(J-3)*P, 2)
            del data_st_p
            return cov
        else:

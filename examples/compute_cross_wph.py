...
stats = wph_op([data1, data2], cross=True).cpu().numpy()
print(stats.shape, stats)

# Method 2: asking for a WPH object
wph = wph_op([data1, data2], cross=True, ret_wph_obj=True)
...
wph_op.load_model("S11", cross_moments=True)
s11_bis = wph_op([data1, data2], cross=True).cpu().numpy()
